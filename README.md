...
- 当前 PaddleOCR 主链构建资源总量为 120,057,620 bytes（Tesseract 降级/回归资产另计）；两条 OCR 链路均不进入首页首屏请求，也不加入 Service Worker `CORE` 预缓存。`/assets/ocr/` 走 SW network-only，避免再复制约 120 MB 到 Cache Storage（浏览器仍可按 HTTP 头缓存）。生产 Pages 已用同一真实长图复验到 15 条候选、10 条自动匹配、5 条人工核对；Android、iOS/已安装 PWA 实机仍为 `NOT_RUN`。
