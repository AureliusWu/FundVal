...
export function calculateOverseasEstimate(model, quotes, now = new Date()) {
  ...
  for (const leg of legs) {
    ...
    if (quote.time) {
      const sourceMs = Date.parse(String(quote.time).replace(' ', 'T') + '+08:00');
      if (Number.isFinite(sourceMs)) {
        sourceTimes.push({ text: quote.time, timestamp: sourceMs });
        if (now.getTime() - sourceMs > 36 * 60 * 60000) stale = true;
      }
    }
  }
  ...
  return {
    ...
    confidence: stale || oldQuarter ? 'low' : (model.confidence || 'medium'),
    stale,
    sourceTime: latestSource?.text || '',
    reason: stale ? '成分行情过期' : '模型可用',
  };
}
