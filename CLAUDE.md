...
本项目当前版本为 `12.0.2`。保持纯前端和零第三方运行时依赖。启动必须经过 `bootstrap.js`，先执行迁移、状态完整性检查和运行时保护，再加载 `app.js`。业务逻辑应按职责进入 `config`、`calculator`、`storage`、`freshness`、`eastmoney-estimate`、`fund-holdings`、`holdings-estimate`、`overseas-model`、`accuracy` 模块，`app.js` 只承担编排和浏览器适配。可测试的修复规则放在 `integrity.js`，浏览器侧恢复与提示放在 `resilience.js`。
...
静态项目无构建流程。修改入口、manifest、SW 或主 JS 时，记得同步 bump `APP_VERSION` 和 `CACHE`。
