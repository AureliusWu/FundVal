...
本项目当前版本为 `10.0.1`。保持纯前端和零第三方运行时依赖。业务逻辑应按职责进入 `config`、`calculator`、`storage`、`overseas-model`、`accuracy` 模块，`app.js` 只承担编排和浏览器适配。海外模型配置只允许在 `data/overseas-models.json` 维护。
